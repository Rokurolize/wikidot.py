...
- Keep using `Page.create_or_edit()`, `Page.refresh_source()`, and `Page.set_metadata()` separately. Local commit `087312d` now wraps these primitives in `site.page.publish(...)` for the common workflow, local commit `ebb3434` adds an opt-in source normalization hook for verification, and local commit `bfe364c` adds opt-in post-save page visibility polling.
