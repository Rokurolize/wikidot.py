...
class TestSiteApplicationAcquireAll:
    ...
    def test_acquire_all_ignores_login_marker_inside_application_text(self):
        """申請本文内のログイン誘導文字列を権限エラーとして扱わない"""
        mock_client = create_mock_client(is_logged_in=True)
        site = _site(mock_client)
            ...
            "body": """
                <div>
                    ...
                    <table>
                        <tr>
                            <td>Application text:</td>
                            <td>WIKIDOT.page.listeners.loginClick(event)</td>
                        </tr>
                    </table>
                </div>
            """
        ...
        assert len(applications) == 1
        assert applications[0].user == mock_user
        assert applications[0].text == "WIKIDOT.page.listeners.loginClick(event)"
    ...
    def test_acquire_all_malformed_response_payload_type_includes_site_context(self):
        """申請リスト応答のpayload型異常はサイト名と型付きで失敗する"""
        mock_client = create_mock_client(is_logged_in=True)
        site = _site(mock_client)
        site.unix_name = "test-site"
        ...
        with (
            ...
            pytest.raises(
                NoElementException,
                match=(
                    "Site application list response payload is malformed for site: test-site "
...
class TestSiteApplicationProcess:
    ...
    def test_accept_malformed_action_response_type_includes_site_user_event_type_and_type_context(self):
        """申請承認応答のpayload型不正は文脈付きNoElementException"""
        mock_client = create_mock_client(is_logged_in=True)
        site = _site(mock_client)
        site.unix_name = "test-site"
        cached_members = [object()]
        ...
        with pytest.raises(
            ...
            match=(
                r"Site application action response is malformed for site: test-site, user: TestUser "
                r"\(id=12345, event=acceptApplication, type=accept, expected=dict, actual=list\)"
            ),
        ):
            app.accept()
