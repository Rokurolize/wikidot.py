...
class TestSiteMemberChangeGroup:
    ...
    def test_change_group_malformed_action_status_type_includes_site_user_event_and_type_context(self):
        """権限変更応答のstatus型不正は文脈付きNoElementException"""
        site = _site()
        site.unix_name = "test-site"
        cached_moderators = [object()]
        cached_admins = [object()]
        ...
        with pytest.raises(
            ...
            match=(
                r"Site member action response is malformed for site: test-site, user: TestUser "
                r"\(id=12345, event=toModerators, field=status, expected=str, actual=list\)"
            ),
        ):
            member.to_moderator()
