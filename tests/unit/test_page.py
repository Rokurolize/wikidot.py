...
class TestPageCollectionSearchPages:
    ...
    def test_search_pages_malformed_first_response_payload_type_includes_site_offset_and_type_context(
        self, mock_site_no_http: Site
    ) -> None:
        """初回ListPages応答のpayload型異常はsite/offset/type付きで失敗する"""
        mock_response = MagicMock()
        ...
        with pytest.raises(
            exceptions.NoElementException,
            match=(
                "ListPages response payload is malformed for site: test-site, offset: 500 "
    ...
    def test_search_pages_malformed_additional_response_payload_type_includes_site_offset_and_type_context(
        self, mock_site_no_http: Site, page_listpages_single: dict[str, Any]
    ) -> None:
        """追加ListPages応答のpayload型異常はsite/offset/type付きで失敗する"""
        first_response = MagicMock()
        first_response.json.return_value = {
            **page_listpages_single,
            "body": page_listpages_single["body"]
        ...
        with pytest.raises(
            exceptions.NoElementException,
            match=(
                "ListPages response payload is malformed for site: test-site, offset: 600 "
                "\\(expected=dict, actual=list\\)"
            ),
        ):
            PageCollection.search_pages(mock_site_no_http, SearchPagesQuery(offset=500, perPage=100))
...
class TestPageCollectionAcquire:
    ...
    def test_acquire_revisions_malformed_response_payload_type_includes_site_page_and_type_context(
        self, mock_site_no_http: Site, mock_page_with_id: Page
    ) -> None:
        """revision一覧応答payloadが辞書でない場合はsite/page/id/type文脈付きで失敗する"""
        collection = PageCollection(mock_site_no_http, [mock_page_with_id])
        malformed_response = MagicMock()
        malformed_response.json.return_value = ["not", "a", "mapping"]
        mock_site_no_http.amc_request = MagicMock()
        mock_site_no_http.amc_request_with_retry = MagicMock(return_value=(malformed_response,))

        with pytest.raises(
            exceptions.NoElementException,
            match=(
                rf"Page revision list response payload is malformed for site: test-site, "
                rf"page: test-page \(id={mock_page_with_id.id}, expected=dict, actual=list\)"
            ),
        ):
            collection.get_page_revisions()
    ...
    def test_acquire_votes_malformed_response_payload_type_includes_site_page_and_type_context(
        self, mock_site_no_http: Site, mock_page_with_id: Page
    ) -> None:
        """vote応答payloadが辞書でない場合はsite/page/id/type文脈付きで失敗する"""
        collection = PageCollection(mock_site_no_http, [mock_page_with_id])
        malformed_response = MagicMock()
        malformed_response.json.return_value = ["not", "a", "mapping"]
        mock_site_no_http.amc_request = MagicMock()
        ...
        with pytest.raises(
            exceptions.NoElementException,
            match=(
                rf"Page vote response payload is malformed for site: test-site, "
                rf"page: test-page \(id={mock_page_with_id.id}, expected=dict, actual=list\)"
            ),
        ):
            collection.get_page_votes()
    ...
    def test_acquire_files_malformed_response_payload_type_includes_site_page_and_type_context(
        self, mock_site_no_http: Site, mock_page_with_id: Page
    ) -> None:
        """file応答payloadが辞書でない場合はsite/page/id/type文脈付きで失敗する"""
        collection = PageCollection(mock_site_no_http, [mock_page_with_id])
        malformed_response = MagicMock()
        malformed_response.json.return_value = ["not", "a", "mapping"]
        mock_site_no_http.amc_request = MagicMock()
        ...
        with pytest.raises(
            exceptions.NoElementException,
            match=(
                rf"Page file response payload is malformed for site: test-site, "
                rf"page: test-page \(id={mock_page_with_id.id}, expected=dict, actual=list\)"
            ),
        ):
            collection.get_page_files()
...
class TestPageProperties:
    ...
    def test_discussion_malformed_response_payload_type_includes_page_context(self, mock_page_with_id: Page) -> None:
        """コメントスレッドIDレスポンスのpayload型異常はsite/page/id/type付きで失敗する"""
        mock_response = MagicMock()
        mock_response.json.return_value = ["not", "a", "mapping"]
        mock_page_with_id.site.amc_request = MagicMock()
        mock_page_with_id.site.amc_request_with_retry = MagicMock(return_value=(mock_response,))
        ...
        with pytest.raises(
            exceptions.NoElementException,
            match=rf"Page discussion response payload is malformed for site: test-site, page: test-page "
            rf"\(id={mock_page_with_id.id}, expected=dict, actual=list\)",
        ):
            _ = mock_page_with_id.discussion
...
class TestPageWriteMethods:
    ...
    def test_metas_getter_malformed_response_payload_type_includes_page_context(self, mock_page_with_id: Page) -> None:
        """metaタグレスポンスのpayload型異常はsite/page/id/type付きで失敗する"""
        mock_response = MagicMock()
        mock_response.json.return_value = ["not", "a", "mapping"]
        mock_page_with_id._metas = None
        mock_page_with_id.site.amc_request = MagicMock()
        ...
        with pytest.raises(
            exceptions.NoElementException,
            match=rf"Page metas response payload is malformed for site: test-site, page: test-page "
            rf"\(id={mock_page_with_id.id}, expected=dict, actual=list\)",
        ):
            _ = mock_page_with_id.metas
