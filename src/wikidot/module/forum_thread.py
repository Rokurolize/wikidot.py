...
def _parse_thread_detail_post_count(
    ...
    post_count_match = re.fullmatch(r"(?:Number of posts:\s*)?(-?[0-9]+)", value_text)
...
def _require_forum_thread_action_status(thread: "ForumThread", event: str, data: object) -> str:
...
class ForumThreadCollection(list["ForumThread"]):
    ...
    def _thread_list_response_body(response: Any, category: "ForumCategory", page: int) -> str:
        ...
        if not isinstance(data, dict):
            raise NoElementException(
                "Forum thread list response payload is malformed "
